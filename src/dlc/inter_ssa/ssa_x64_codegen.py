...
from dlc.inter_ssa.ssa_reg_alloc import SSALinearScanRegisterAllocator
...
class SSAX64CodeGenerator():
    ...
    def __init__(self, ssa: SSA):
        ...
        live_analysis = LivenessAnalysis(ssa)
        # Vivacidade apenas de inteiros
        int_live_ranges = { temp: range 
            for temp, range in live_ranges.items() 
            if temp.type.is_integral or temp.type.is_boolean}
        # Vivacidade apenas de ponto flutuante
        double_live_ranges = { temp: range 
            for temp, range in live_ranges.items() 
            if temp.type.is_float}


        #int_scan_reg_alloc = SSALinearScanRegisterAllocator(int_live_ranges, self.INT_REGISTERS)
        double_scan_reg_alloc = SSALinearScanRegisterAllocator(double_live_ranges, self.DOUBLE_REGISTERS)
        #int_reg_alloc = int_scan_reg_alloc.allocations
        double_reg_alloc = double_scan_reg_alloc.allocations
        #int_spill_loc = int_scan_reg_alloc.spills
        double_spill_loc = double_scan_reg_alloc.spills
            ...
            match instr.op:
                ...
                case _:
                    if instr.op in (SSAOperator.SUM, SSAOperator.SUB, SSAOperator.MUL):
                        ...
                        self.code.append(f'\t{self.MOVE[result_type]} {result}, {self.ACC_REG[type]}')
                    elif instr.op in (SSAOperator.EQ, SSAOperator.NE, SSAOperator.LT, SSAOperator.LE, SSAOperator.GT, Operator.GE):
                        self.code.append(f'\t{self.MOVE[type]} {self.ACC_REG[type]}, {arg1}')
...
from dlc.inter_ssa.ssa import SSA
from dlc.inter_ssa.ssa_live_range import LivenessAnalyzer
from dlc.inter_ssa.ssa_operator import SSAOperator
from dlc.inter_ssa.ssa_reg_alloc import SSALinearScanRegisterAllocator
from dlc.semantic.type import Type
