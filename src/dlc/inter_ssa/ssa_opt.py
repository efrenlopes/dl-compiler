...
def constant_folding(ssa: SSA) -> bool:
    ...
    for bb in ssa.ic.bb_sequence:
        ...
        for instr in bb.instructions:
            ...
            if instr.op in SSA_IC.OPS_BIN and instr.arg1.is_const and instr.arg2.is_const:
                try:
                    # 2. Calcular o resultado em tempo de compilação
                    val1 = instr.arg1.value
                    val2 = instr.arg2.value
                    result_value = SSA_IC.OPS_BIN[instr.op](val1, val2)
                    ...
                    pass
            elif instr.op in SSA_IC.OPS_UN and instr.arg1.is_const:
                val1 = instr.arg1.value
                result_value = SSA_IC.OPS_UN[instr.op](val1)
                instr.op = SSAOperator.MOVE
                instr.arg1 = SSAConst(instr.result.type, result_value)
                instr.arg2 = SSAOperand.EMPTY
                changed = True
        ...
        for instr in bb.instructions:
            ...
            if instr.op == SSAOperator.PHI:
                for version in instr.arg1.paths.values():
                    if hasattr(version, 'is_temp_version') and version.is_temp_version:
