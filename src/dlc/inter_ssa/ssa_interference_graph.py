...
class InterferenceGraph:
    def __init__(self, liveness: LivenessAnalysis, num_registers: int):
    ...
    def __color_graph(self):
        ...
        available_regs = [i for i in range(self.num_registers)]
        ...
        while nodes:
            # Busca nó com grau < K
            for node in nodes:
                if len(temp_graph[node]) < self.num_registers:
                    stack.append(node)
