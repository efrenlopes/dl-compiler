...
tags_seq = (Tag.PROGRAM, Tag.ID, Tag.BEGIN,
            ...
            Tag.ID, Tag.ASSIGN, Tag.LPAREN, Tag.ID, Tag.GT, Tag.LIT_INT, Tag.OR,
            Tag.ID, Tag.EQ, Tag.LIT_INT, Tag.RPAREN, Tag.SEMI,
            Tag.IF, Tag.LPAREN, Tag.ID, Tag.EQ, Tag.LIT_TRUE, Tag.RPAREN, Tag.BEGIN,
            Tag.REAL, Tag.ID, Tag.SEMI, Tag.ID, Tag.ASSIGN, Tag.LIT_REAL, Tag.MUL,
            Tag.ID, Tag.MUL, Tag.ID, Tag.SEMI,
...
def test_lexer() -> None:
    ...
    while i < len(tags_seq) and token.tag != Tag.EOF:
        assert token.tag == tags_seq[i], \
                  f'Erro no token {i}: esperado "{tags_seq[i].name}", \
                    recebido "{token.tag.name}"'
        token = lexer.next_token()
        i += 1
